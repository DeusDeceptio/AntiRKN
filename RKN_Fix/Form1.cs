...
namespace AntiRKN
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            toolTip1.SetToolTip(this.buttonSearchConf, "Àêòèâíà, åñëè â âàøåé ñáîðêå åñòü \"ÀÂÒÎ - ÏÎÈÑÊ ïðå - êîíôèãà.exe\"");
            toolTip1.SetToolTip(this.buttonAutoConfig, "Àêòèâíà, åñëè â âàøåé ñáîðêå åñòü \"Óñòàíîâèòü êàê ñëóæáó â ÀÂÒÎÇÀÏÓÑÊ v2.exe\"");
            toolTip1.SetToolTip(groupBox1, "Âûáîð ñïèñêîâ äîñòóïåí äëÿ ñáîðîê ïîääåðæèâàþùèõ ýòó ôóíêöèþ");
        }
        ...
        private void îòêðûòüÌåíþToolStripMenuItem_Click(object sender, EventArgs e)
        {
            openForm();
        }
        ...
        private void buttonStartZap_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Âûáåðåòå êîíôèã!");
                return;
            }
            try
            {
        ...
        private void ñäåëàòüÏîÓìîë÷àíèþToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(textBoxSelect.Text))
            {
            ...
            {
                MessageBox.Show("Êîíôèã íå âûáðàí!");
            }
        }
        ...
        private void startDefConf()
        {
            ...
            {
                MessageBox.Show("Êîíôèã ïî óìîë÷àíèþ íå âûáðàí! \nÍàæìèòå ËÊÌ ïî ïîëþ ñ âûáðàííûì êîíôèãîì.");
            }
        }
        ...
        private void buttonOpenExplorer_Click(object sender, EventArgs e)
        {
            ...
            dialog.Filter = "Èñïîëíèòåëüíûé ôàéë (*.exe)|*.exe|Áàòíèêè (*.bat)|*.bat|Âñå ôàéëû (*.*)|*.*";
        ...
        {
            if (MessageBox.Show("Ïîäòâåðäèòå óäàëåíèå çàïèñè.", "", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
            int index = listBoxVPN.SelectedIndex;
            if (index < 0) return;
        ...
        {
            if (MessageBox.Show("Ïîäòâåðäèòå ñáðîñ íàñòðîåê.", "", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
            otherVPNList.Clear();
        ...
        {
            if (!File.Exists(Application.StartupPath + "ÀÂÒÎ-ÏÎÈÑÊ ïðå-êîíôèãà.exe")) return;
            try
            {
                Process.Start(Application.StartupPath + "ÀÂÒÎ-ÏÎÈÑÊ ïðå-êîíôèãà.exe");
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
        ...
        {
            if (!File.Exists(Application.StartupPath + "Óñòàíîâèòü êàê ñëóæáó â ÀÂÒÎÇÀÏÓÑÊ v2.exe")) return;
            try
            {
                Process.Start(Application.StartupPath + "Óñòàíîâèòü êàê ñëóæáó â ÀÂÒÎÇÀÏÓÑÊ v2.exe");
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
    }
